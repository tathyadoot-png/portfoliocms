...
- Schema is **migration-based** (`supabase/migrations/`) — migrations are the sole source of truth for schema history. Twelve migrations exist today (`20260817120001` through `20260817120010`, plus `20260825120011` and `20260825180012` which only GRANT table privileges to `authenticated` — no table/column/enum/RLS-policy changes).
...
- **Table GRANTs are also required.** New Supabase projects do not auto-expose `public` tables to Data API roles. `authenticated` has `SELECT/INSERT/UPDATE/DELETE` on the implemented CMS tables (`portfolios`, `activities`, `media`, `cloudinary_configs`, `portfolio_settings`, `social_links`). `anon` is revoked. `seo_meta` stays un-granted (SEO UI is not built). RLS policies are never evaluated if the GRANT is missing (PostgREST 403: `permission denied for table …`). Applied in migrations `20260825120011` and `20260825180012` (social_links only).
...
- **A public-facing/anon-readable API** — `anon` currently has zero RLS access to any table; opening read access for the public websites is a future, separate decision that will require new RLS policies (not schema changes).
...
## 14. Verification history (accurate as of the last completed phase, Phase 7)
...
- Hosted GRANT check: `authenticated` can SELECT/INSERT/UPDATE/DELETE `social_links`; `anon` remains denied; `seo_meta` remains un-granted.
- Hosted DML (rolled back): create two links, hide, reorder, delete — leftover 0.
- In-browser CMS walkthrough of Social Links was not run in this session (no authenticated browser session).
